...
exports.register = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error in registering user', details: error.message });
  }
};
...
// Logout user (if implementing server-side)
exports.logout = async (req, res) => {
  try {
    // Note: Since JWT is stateless, server-side logout is optional
    // You might want to implement a token blacklist here if needed
    
    res.json({ message: 'Logged out successfully' });
  } catch (error) {
    res.status(500).json({ message: 'Error in logging out', error: error.message });
  }
};
